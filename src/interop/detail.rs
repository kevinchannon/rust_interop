...
pub fn null_term_chars_from_str(s: String, buf: *mut c_char, max_len: size_t) -> ResultCode {
...
fn user_list<'a>() -> &'a mut Mutex<UserList> {
    INIT.call_once(|| {
    ...
    unsafe { STD_ONCE_COUNTER.as_mut().unwrap() }
}
...
pub fn get_user_name<'a>(h: &UserHandle) -> String {
  match user_list().lock().unwrap().iter().position(|u| u.id == *h){
    Some(pos) => {
      user_list().lock().unwrap()[pos].name.clone()
    },
    None => {
      "".to_string()
    }
...
mod tests {
  ...
  fn set_and_get_user_name_with_valid_handle_returns_name(){
      ...
      let h = create_user(128);
      println!("Setting user name");
      assert_eq!(RC_OK, set_user_name(&h, "human 1"));
      println!("Gettinh user name");
      assert_eq!("human 1", get_user_name(&h));
      println!("Done");
  } 
}
